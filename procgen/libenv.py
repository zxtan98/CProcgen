...
from gym3 import libenv as libenv_orig
...
def _load_libenv_cdef():
    libenv_cdef = ""
    # get the path of gym3
    with open(os.path.join(get_header_dir(), 'src', "libenv.h")) as f:
        inside_cdef = False
        for line in f:
...
class CEnv(Env):
    ...
    def _convert_contexts(self, ffi: Any, c_lib: Any, context_options: List) -> Any:
        keepalives = []
        env_num = len(context_options)
        c_contexts = ffi.new("struct libenv_options [{}]".format(env_num))
